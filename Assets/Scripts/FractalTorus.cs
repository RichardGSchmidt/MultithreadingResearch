...
public class FractalTorus : MonoBehaviour {
    ...
    public void GenerateFractalTorus(ModuleBase baseModIn, float adjustment) {
        ...
        for (int seg = 0; seg <= nbRadSeg; seg++)
        {
            ...
            for (int side = 0; side <= nbSides; side++)
            {
                ...
                Vector3 valueVert = r1 + r2;
                //note injection on following line
                vertices[side + seg * (nbSides + 1)] = valueVert+adjustment*.1f*valueVert*(float)baseModIn.GetValue(valueVert);
            }
        }
        #endregion
        ...
        #region Custom Noise Map Adjust Demo
